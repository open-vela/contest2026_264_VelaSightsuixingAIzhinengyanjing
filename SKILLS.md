...
> 本文件是本仓库唯一的项目级交付规范，路径以 `/home/mi/vela_competition`
> 工作区根目录为基准，在工作区根通用规则之上补充 `external` 完整文件 overlay、
> 构建与验收要求。
>
> 此前 `docs/SKILLS.md` 与仓库根各存一份，内容分叉：根目录那份停在只读 SDIO 和
> `bk7258-ap_nsh` 构建目录，且缺少 3.7 节的启动堆预算。两份同名而一份过时，
> 读者没有办法判断该信任哪一份，所以现在只保留这一份，`docs/SKILLS.md` 已删除。
