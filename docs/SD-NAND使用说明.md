...
> **性能红线：不要在 UI 渲染、按键处理、动画、音视频实时回调或其他延迟敏感热路径中执行 SD-NAND/NAND 读写。**
> 当前 SD-NAND 使用 1-bit PIO 和单块传输，文件读取或写入可能产生极高、接近秒级的阻塞延迟。
> 持久配置应在启动阶段或后台工作线程中读取并缓存；热路径只能访问内存中的快照，写入应异步批处理。
