...
check is read-only. install validates the complete 59-file overlay first, then
  ...
  /^[[:space:]]*$/ || /^#/ { next }
  {
    ...
    if ($1 == "repo") {
      ...
      if ($2 == "apps") {
        ...
        if ($3 != "packages/ai_agent" || $4 != "packages/ai_agent" || $7 != "18")
          fail("packages_ai_agent mapping/count must be packages/ai_agent with 18 files")
