    ...
    def apps_info(self) -> list[app_info]:
        ...
        for app_name in self._project_info.apps:
            if app_name.endswith("_ap"):
                ...
                external_ap = os.environ.get("EXTERNAL_AP_BIN")
                if external_ap:
                    app_build_bin = Path(external_ap).expanduser().resolve()
                    if not app_build_bin.is_file():
                        raise FileNotFoundError(
                            f"EXTERNAL_AP_BIN does not exist: {app_build_bin}"
                        )
                    if app_build_bin.stat().st_size == 0:
                        raise RuntimeError(
                            f"EXTERNAL_AP_BIN is empty: {app_build_bin}"
                        )
                else:
