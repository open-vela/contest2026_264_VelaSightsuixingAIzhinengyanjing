...
#define MB_UART_STATUS_TX_FAIL		0x04
#define MB_UART_STATUS_PROTOCOL_ERR	0x08

typedef enum {
	MB_UART_EVENT_READY = 0,
	MB_UART_EVENT_DOWN,
	MB_UART_EVENT_PEER_RESET,
	MB_UART_EVENT_TIMEOUT,
} mb_uart_event_t;

typedef enum {
	MB_UART_LINK_DOWN = 0,
	MB_UART_LINK_ABORTING,
	MB_UART_LINK_PROBING,
	MB_UART_LINK_READY,
} mb_uart_link_state_t;

typedef void (* mb_uart_isr_t)(void * param);
typedef void (* mb_uart_event_cb_t)(u8 id, mb_uart_event_t event, void *param);
 ...
 *
 * @param id: mailbox uart ID (MB_UART0, MB_UART1).
 *
 * @return
 ...
 * @param id: mailbox uart ID (MB_UART0, MB_UART1).
 *
 * @param isr: rx isr, prototype is void (* mb_uart_isr_t)(void * param)
 *
 * @param param: will be passed to rx isr callback.
 *
 ...
 * @param id: mailbox uart ID (MB_UART0, MB_UART1).
 *
 * @param isr: tx isr, prototype is void (* mb_uart_isr_t)(void * param)
 *
 * @param param: will be passed to tx isr callback.
 *
 ...
 * @param data_len: data bytes in buffer.
 *
 * @return
 ...
 *
 * @param id: mailbox uart ID (MB_UART0, MB_UART1).
 * @param data: data buffer to receive one byte.
 *
 * @return
 ...
 *
 * @param id: mailbox uart ID (MB_UART0, MB_UART1).
 * @param data_buf: used to receive data.
