...
static void wpas_trigger_scan_cb(struct wpa_radio_work *work, int deinit)
{
	...
	WPA_LOGV("scan work start, deinit=%d\n", deinit);
	if (deinit) {
		if (!work->started) {
	...
	if (ret) {
		...
		wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_SCAN_FAILED "ret=%d%s",
			ret, retry ? " retry=1" : "");
		WPA_LOGD("req driver scan failed, ret=-0x%x, %s\n",
			-ret, retry ? "retry" : "");
		radio_work_done(work);
...
void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (connect_without_scan) {
		...
		if (ssid) {
			...
			WPA_LOGD("%s: connect_without_scan set but cannot find wpa_ssid\n");
		}
	}
	...
	if (wpa_s->last_scan_req == MANUAL_SCAN_REQ &&
		...
		WPA_LOGD("use ssids from scan command\n");
...
ssid_list_set:
	...
	if (bk_feature_bssid_connect_enable()) {
		if (!is_zero_ether_addr(g_sta_param_ptr->fast_connect.bssid)) {
			params.bssid = g_sta_param_ptr->fast_connect.bssid;
			WPA_LOGD("Scan specified BSSID " MACSTR  "\r\n", MAC2STR(params.bssid));
		}
	}
 ...
 * the specified time.
 */
int wpa_supplicant_req_scan(struct wpa_supplicant *wpa_s, int sec, int usec)
{
	int res;
	...
	if (res == 1) {
		...
		WPA_LOGD("Rescheduling scan request: %d.%06d sec\n", sec, usec);
#endif
	} else if (res == 0) {
		wpa_dbg(wpa_s, MSG_DEBUG, "Ignore new scan request for %d.%06d sec since an earlier request is scheduled to trigger sooner",
			sec, usec);
#ifdef CONFIG_NO_STDOUT_DEBUG
		WPA_LOGD("Ignore new scan request for %d.%06d sec since an earlier request is scheduled to trigger sooner\n",
			sec, usec);
#endif
	} else {
...
{
	WPA_LOGV("cancell scan request timer\r\n");
 ...
 * better. */
int wpa_scan_result_compar(const void *a, const void *b)
{
#ifndef MIN
...
static const struct minsnr_bitrate_entry he20_table[] __maybe_unused = {
	{ 0, 0 },
	{ 2, 8600 },    /* HE20 MCS0 */
	{ 5, 17200 },   /* HE20 MCS1 */
	{ 9, 25800 },   /* HE20 MCS2 */
...
static const struct minsnr_bitrate_entry he40_table[] __maybe_unused = {
	{ 0, 0 },
	{ 5, 17200 },   /* HE40 MCS0 */
	{ 8, 34400 },   /* HE40 MCS1 */
	{ 12, 51600 },  /* HE40 MCS2 */
...
static const struct minsnr_bitrate_entry he80_table[] __maybe_unused = {
	{ 0, 0 },
	{ 8, 36000 },   /* HE80 MCS0 */
	{ 11, 72100 },  /* HE80 MCS1 */
	{ 15, 108100 }, /* HE80 MCS2 */
...
static const struct minsnr_bitrate_entry he160_table[] __maybe_unused = {
	{ 0, 0 },
	{ 11, 72100 },   /* HE160 MCS0 */
...
#ifdef BK_SUPPLICANT
	/*
	 * if not disconnect manually, req scan.
	 */
	if (!wpa_s->disconnected &&
		wpa_s->wpa_state <= WPA_SCANNING &&
