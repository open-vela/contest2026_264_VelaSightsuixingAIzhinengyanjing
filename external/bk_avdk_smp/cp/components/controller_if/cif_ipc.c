...
extern bk_err_t cif_dnld_buffer(void *param, void *node);
...
bk_err_t cif_ipc_init()
{
    ...
    for(uint8_t i=0; i<IPC_MAX; i++)
    {
        ...
        ret = cif_ipc_env[i].cb_register(cif_ipc_env[i].channel,
    ...
    /* CP event slots cannot be reset from MB_CHNL_EVENT_PEER_RESET: that
     * callback runs in interrupt context while event allocation is task-owned
     * and has no shared lock.  Orphaned slots are recovered by cif_init().
     */
