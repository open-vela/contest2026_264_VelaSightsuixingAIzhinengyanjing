...
static bk_err_t cif_msg_sender_list(void* head, enum cif_task_msg_evt type,
                                    uint8_t retry, uint8_t num)
{
    bk_err_t ret = BK_OK;
    struct cif_msg msg;
...
{
    return cif_msg_sender_list(head, type, retry, 1);
}
void cif_main(void *arg)
{
    ...
    while (1)
    {
        ...
        switch (msg.type)
        {
            ...
            {
                void *head = (void*)msg.arg;
                for (uint8_t i = 0; i < msg.num && head != NULL; i++)
                {
                    void *next = (void*)((struct cpdu_t*)head)->next;
                    ret = cif_handle_bk_cmd(head);
                    head = next;
                }
                break;
            }
...
bk_err_t cif_dnld_buffer(void *param, void *payload)
{
    bk_err_t ret = BK_OK;
    ...
    CIF_LOGV("%s,chan_id=%d,head=0x%x,tail=0x%x,num=%d,start!\n",
             __func__,chan_id,head,node->tail,num);
    ...
    switch(chan_id)
    {
        case TX_BK_CMD_DATA:
        {
            ret = cif_msg_sender_list(head,CIF_TASK_MSG_CMD,0,num);
            break;
        }
        case TX_MSDU_DATA:
        {
            ret = cif_msg_sender_list(head,CIF_TASK_MSG_DATA,0,num);
            if (ret == BK_OK)
            {
                cif_stats_ptr->cif_tx_dnld_cnt += num;
            }
            break;
        }
        default:
