...
bk_err_t pm_core_bus_clock_ctrl(uint32_t cksel_core, uint32_t ckdiv_core, uint32_t ckdiv_bus, uint32_t ckdiv_cpu0, uint32_t ckdiv_cpu1)
{
	...
	GLOBAL_INT_DISABLE();
	ret = sys_drv_core_bus_clock_ctrl(cksel_core, ckdiv_core, ckdiv_bus, ckdiv_cpu0, ckdiv_cpu1);
	GLOBAL_INT_RESTORE();
...
{
	if (module >= PM_DEV_ID_MAX || cpu_freq > PM_CPU_FRQ_DEFAULT)
		return BK_ERR_PARAM;
	...
	bk_err_t ret = BK_OK;
	pm_cpu_freq_e previous_vote;
	uint32_t i = 0;
	uint32_t freq_max = 0;
	uint32_t freq_max_index = 0;
	bool high_voltage_target;
	...
	{
		high_voltage_target = (freq_max == PM_CPU_FRQ_480M) ||
			(freq_max == PM_CPU_FRQ_320M);
		if(high_voltage_target)
		{
			ret = bk_pm_module_vote_vdddig_ctrl(PM_VDDDIG_MODULE_CPU_FREQ,
	...
	if (ret == BK_OK)
	{
		...
		if (!high_voltage_target)
			bk_pm_module_vote_vdddig_ctrl(PM_VDDDIG_MODULE_CPU_FREQ,
				PM_VDDDIG_HIGH_STATE_OFF);
	}
	else
	{
		s_pm_cpu_freq[module] = previous_vote;
		if (s_pm_current_cpu_freq != PM_CPU_FRQ_480M &&
			s_pm_current_cpu_freq != PM_CPU_FRQ_320M)
			bk_pm_module_vote_vdddig_ctrl(PM_VDDDIG_MODULE_CPU_FREQ,
				PM_VDDDIG_HIGH_STATE_OFF);
	}
