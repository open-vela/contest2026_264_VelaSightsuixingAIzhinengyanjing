...
#define ATE_GPIO_DEV_UART0_RXD       10
#define ATE_GPIO_DEV_UART0_TXD       11
...
void gpio_voltage_output_high(void)
{
	...
	{
		if(i == ATE_GPIO_DEV_UART0_RXD || i == ATE_GPIO_DEV_UART0_TXD)
			continue;
...
#if (CONFIG_SOC_BK7231N) || (CONFIG_SOC_BK7236XX) || (CONFIG_SOC_BK7239XX) || (CONFIG_SOC_BK7286XX) || (CONFIG_SOC_BK7256XX)
...
#elif (CONFIG_SOC_BK7256XX) || (CONFIG_SOC_BK7236XX) || (CONFIG_SOC_BK7239XX) || (CONFIG_SOC_BK7286XX)
...
int ate_enter_low_voltage()
{
	...
	BK_LOGE(NULL, "ATE AP low-voltage control is disabled for OpenVela SMP\r\n");
...
{
#if CONFIG_OPENVELA_AP_480M
	BK_LOGE(NULL, "ATE AP deep-sleep control is disabled for OpenVela SMP\r\n");
	return BK_ERR_NOT_SUPPORT;
#else
	bk_ate_ctrl(0);
	bk_pm_module_vote_power_ctrl(PM_POWER_MODULE_NAME_BTSP,POWER_MODULE_STATE_OFF);
