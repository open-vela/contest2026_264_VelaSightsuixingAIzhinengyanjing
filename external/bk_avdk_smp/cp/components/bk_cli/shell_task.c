...
static void rx_ind_process(void)
{
	...
	if( cmd_rx_done )
	{
		if(cmd_line_buf.cur_cmd_type == CMD_TYPE_TEXT)
		{
			...
			if( cmd_line_buf.cmd_data_len > 0 )
			{
				extern void ap_uart0_rx_forward(const uint8_t *data, uint16_t length);
				ap_uart0_rx_forward((const uint8_t *)cmd_line_buf.cmd_buff, (uint16_t)cmd_line_buf.cmd_data_len);
