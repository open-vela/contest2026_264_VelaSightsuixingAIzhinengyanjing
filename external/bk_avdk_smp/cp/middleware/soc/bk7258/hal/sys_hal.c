...
void sys_hal_module_power_ctrl(power_module_name_t module,power_module_state_t power_state)
{
	...
	if((module >= POWER_MODULE_NAME_MEM1) && (module <= POWER_MODULE_NAME_ROM_PGEN)) {
		...
		if(power_state == POWER_MODULE_STATE_ON) {
			...
			/*2. halt*/
			sys_ll_set_cpu2_int_halt_clk_op_cpu2_halt(0);

			/*3.wait halt really cleared,clock support finish*/
			for(int i = 0; i < 1000; i++);
		} else {
			/*halt and then power down*/
			/*1. halt*/
			sys_ll_set_cpu2_int_halt_clk_op_cpu2_halt(1);

			/*2.here should wait halt really finish and then power down*/
			for(int i = 0; i < 1000; i++);
...
int32 sys_hal_module_power_state_get(power_module_name_t module)
{
	...
	if((module >= POWER_MODULE_NAME_MEM1) && (module <= POWER_MODULE_NAME_ROM_PGEN)) {
		...
		return sys_ll_get_cpu_current_run_status_cpu2_pwr_dw_state();
	} else if (module == POWER_MODULE_NAME_TCM1_PGEN) {
		/*TODO
...
{
    sys_ll_set_cpu2_int_halt_clk_op_cpu2_offset(address_offset);
}

void sys_hal_set_cpu1_reset(uint32_t reset_value)
{
	/* 1 releases reset; 0 holds reset. Verified through run-status readback. */
...
{
	/* 1 releases reset; 0 holds reset. Verified through run-status readback. */
	sys_ll_set_cpu2_int_halt_clk_op_cpu2_sw_rst(reset_value);
}
...
bk_err_t sys_hal_core_bus_clock_ctrl(uint32_t cksel_core, uint32_t ckdiv_core,uint32_t ckdiv_bus, uint32_t ckdiv_cpu0,uint32_t ckdiv_cpu1)
{
	...
	old_clk = sys_hal_all_modules_clk_div_get(CLK_DIV_REG0);
	old_cpu[0] = sys_hal_cpu_clk_div_get(0);
	old_cpu[1] = sys_hal_cpu_clk_div_get(1);
	old_cpu[2] = sys_hal_cpu_clk_div_get(2);
	clk_param = 0;
	...
	if(((clk_param >> 0x4)&0x3) > cksel_core)//when it from the higher frequency to lower frequency
	{
		...
		clk_param = 0;
		clk_param = sys_hal_all_modules_clk_div_get(CLK_DIV_REG0);
		clk_param &= ~((0xF << 0) | (0x1 << 6));
		clk_param |= (ckdiv_core << 0) | (ckdiv_bus << 6);
		sys_hal_all_modules_clk_div_set(CLK_DIV_REG0,clk_param);
	...
	clk_param = sys_hal_all_modules_clk_div_get(CLK_DIV_REG0);
	if (((clk_param >> 4) & 0x3) != cksel_core ||
		((clk_param >> 0) & 0xF) != ckdiv_core ||
		((clk_param >> 6) & 0x1) != ckdiv_bus ||
		sys_hal_cpu_clk_div_get(0) != ckdiv_cpu0 ||
		sys_hal_cpu_clk_div_get(1) != ckdiv_cpu1 ||
		sys_hal_cpu_clk_div_get(2) != 1)
	{
		BK_LOGE(NULL, "dvfs readback mismatch clk=0x%x cpu=%d/%d/%d\r\n",
			clk_param, sys_hal_cpu_clk_div_get(0),
			sys_hal_cpu_clk_div_get(1), sys_hal_cpu_clk_div_get(2));
		sys_hal_all_modules_clk_div_set(CLK_DIV_REG0, old_clk);
		sys_hal_cpu_clk_div_set(0, old_cpu[0]);
		sys_hal_cpu_clk_div_set(1, old_cpu[1]);
		sys_hal_cpu_clk_div_set(2, old_cpu[2]);
		return BK_FAIL;
	}
...
bk_err_t sys_hal_set_vdddig_h_vol(uint32_t target_value)
{
	...
	return sys_ll_get_ana_reg9_vcorehsel() == target_value ? BK_OK : BK_FAIL;
}
bk_err_t sys_hal_ctrl_vdddig_h_vol(uint32_t vol_value)
{
...
bk_err_t sys_hal_switch_cpu_bus_freq_high_to_low(pm_cpu_freq_e cpu_bus_freq)
{
	...
	switch(cpu_bus_freq)
	{
		case PM_CPU_FRQ_480M://cpu0:240m;cpu1:480m;cpu2:480m;bus:240m
			ret = sys_hal_core_bus_clock_ctrl(0x3,0x0,0x1,0x0,0x1);
			if (ret != BK_OK) break;
			...
			sys_hal_ctrl_vddd_h_vol(0x7);//  1.05v
#endif
			sys_hal_set_vdddig_h_vol(bk_pm_vdddig_required_get(cpu_bus_freq));
			break;
		case PM_CPU_FRQ_320M://cpu0:160m;cpu1:320m;cpu2:320m;bus:160m
		    ret = sys_hal_core_bus_clock_ctrl(0x2,0x0,0x1,0x0,0x1);
			if (ret != BK_OK) break;
			sys_hal_ctrl_vddd_h_vol(0x7);// 1.05v
			sys_hal_set_vdddig_h_vol(bk_pm_vdddig_required_get(cpu_bus_freq));
...
bk_err_t sys_hal_switch_cpu_bus_freq_low_to_high(pm_cpu_freq_e cpu_bus_freq)
{
	...
	ret = sys_hal_set_vdddig_h_vol(
		bk_pm_vdddig_required_get(cpu_bus_freq));
	if (ret != BK_OK)
		return ret;
	switch(cpu_bus_freq)
	{
...
bk_err_t sys_hal_switch_cpu_bus_freq(pm_cpu_freq_e cpu_bus_freq)
{
	...
	if (ret == BK_OK && sys_hal_vdddig_h_vol_get() <
		bk_pm_vdddig_required_get(cpu_bus_freq))
		ret = BK_FAIL;
	if (ret != BK_OK)
	{
		bk_err_t rollback_ret;

		if (prev_freq > cpu_bus_freq)
			rollback_ret = sys_hal_switch_cpu_bus_freq_low_to_high(prev_freq);
		else
			rollback_ret = sys_hal_switch_cpu_bus_freq_high_to_low(prev_freq);
		if (rollback_ret != BK_OK)
			BK_LOGE(NULL, "dvfs rollback failed target=%d previous=%d ret=%d\r\n",
				cpu_bus_freq, prev_freq, rollback_ret);
		return ret;
	}
	if (ret == BK_OK)
...
void sys_hal_set_rott_int_en(uint32_t value)
{
	...
	sys_ll_set_cpu0_int_32_63_en_cpu0_rott_int_en(value);
