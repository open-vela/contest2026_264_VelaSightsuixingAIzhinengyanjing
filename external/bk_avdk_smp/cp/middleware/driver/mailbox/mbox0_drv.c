...
static void mbox0_drv_isr_handler(void)
{
	...
	do
	{
		...
		{
			if (mbox0_dev.rx_ready_callback &&
				!mbox0_dev.rx_ready_callback())
				break;
...
void mbox0_drv_poll(void)
{
	mbox0_drv_isr_handler();
}
...
int mbox0_drv_get_send_count(uint32_t dest_cpu, uint32_t *fifo_count)
{
	if ((dest_cpu >= MBOX_CHNL_NUM) || (fifo_count == NULL))
		return MBOX0_HAL_SW_PARAM_ERR;

	*fifo_count = mbox0_dev.chn_drv[dest_cpu]->chn_get_rx_fifo_count(
		&mbox0_dev.hal);
	return MBOX0_HAL_OK;
}
...
int mbox0_drv_ready_callback_register(mbox0_rx_ready_callback_t callback)
{
	mbox0_dev.rx_ready_callback = callback;
	return 0;
}
...
int mbox0_drv_init(void)
{
	...
	mbox0_dev.rx_ready_callback = NULL;
